    ...
    let stubGeocodeService = {
      fetchCoordinates(location) {
        assert.equal(
          location,
          "San Francisco",
          "fetchCoordinates called with location"
        );
        return Promise.resolve([0, 0]);
      },
    };
    let mapService = this.owner
      .factoryFor("service:map-element")
      .create({ map: stubMapService, geocode: stubGeocodeService });
    let element = await mapService.getMapElement("San Francisco");
    assert.ok(element, "element exists");
    assert.equal(element.className, "map", "element has classs name of map");
  });
